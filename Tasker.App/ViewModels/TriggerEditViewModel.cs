...
public partial class TriggerEditViewModel : ObservableObject
{
    ...
    /// <summary>True when at least one weekday box is checked (weekly / monthly-by-weekday).</summary>
    public bool AnyWeekdaySelected => Monday || Tuesday || Wednesday || Thursday || Friday || Saturday || Sunday;

    /// <summary>True when at least one "which week" box is checked (monthly-by-weekday).</summary>
    public bool AnyWeekSelected => WeekFirst || WeekSecond || WeekThird || WeekFourth || WeekLast;

    /// <summary>True when at least one month is checked (monthly triggers).</summary>
    public bool AnyMonthSelected => Jan || Feb || Mar || Apr || MayMonth || Jun || Jul || Aug || Sep || Oct || Nov || Dec;

    /// <summary>True when the "days of month" text yields at least one valid day, or last-day is set.</summary>
    public bool AnyMonthDaySelected => RunOnLastDayOfMonth ||
        DaysOfMonthText.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Any(p => int.TryParse(p, out var d) && d is >= 1 and <= 31);
